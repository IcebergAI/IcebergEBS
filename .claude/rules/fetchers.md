...
- Version extracted via `_find_detail_value(soup, "version", exact=True)` (exact label match, so description prose containing "Version" can't hijack it, #279), validated against `[0-9][0-9.]*`. Real Chrome uses the **inline** `<div>Version: 1.54.0</div>` form (label+value in one node) for Version even though Updated/Offered by use the split label/value form, so exact-label returns empty and an inline path runs. It collects the *visible* text nodes that `re.fullmatch` `Version\s*:\s*x.y.z` (essentially just the version, so an embedding sentence like "New in Version 9.9.9 …" never matches), then chooses structurally: if a Details label ("Updated"/"Offered by") is present, the first candidate **at or after** it wins (the description/What's-new section renders before those labels, so a standalone `Version: 9.9.9` prose node there is out of scope); otherwise (a degraded page with no Details labels — the `#142` head/script-noise fixtures) the **last** candidate wins, since the real version follows any earlier mention. Never reintroduce a plain first-match document-order scan here — an earlier description node hijacks it (#279)
