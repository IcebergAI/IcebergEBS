...
- **ui** (#100) is the **only** gate that exercises the app in a real browser, so it's the only thing that catches a **strict-CSP / Alpine** regression at runtime — an inline `<script>`, an `on*=` handler, or an `x-data` expression the `@alpinejs/csp` build can't evaluate passes every other job but fails here (the console-error backstop, `_KNOWN_CSP_GAPS` empty). It boots the full Compose stack behind Caddy with a self-signed cert (`caddy/generate-dev-cert.sh`), so it also depends on the compose files and that script. Playwright + Chromium are installed in an **ephemeral venv outside `uv.lock`**, at their **own pinned `playwright`/`chromium` versions in `ci.yml`** (bump those together on a browser refresh — they are not Dependabot-managed). Editing the CSP (`caddy/headers.caddy` + the Helm mirror), an Alpine component, or the compose stack means this job is what proves it still works.
