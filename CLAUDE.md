...
- **ui** (#100) is the only gate that drives the app in a **real browser**, so it's the **runtime** backstop for what the static CSP checks can't see. The static side is already covered by the blocking `test` job — `tests/test_csp_strict.py` rejects inline `<script>` and `on*=` handlers across every template and enforces `script-src 'self'` / no `sha256-` pins in **both** `caddy/headers.caddy` **and** the Helm ConfigMap mirror. What only the `ui` job catches is a violation that manifests at runtime: an `x-data`/directive expression the `@alpinejs/csp` build can't evaluate (the component silently fails to initialise), or a dynamically-injected inline script — surfaced as a browser **console error that fails the job** (`_KNOWN_CSP_GAPS` is empty; never add to it). It boots the full Compose stack behind Caddy with a self-signed cert (`caddy/generate-dev-cert.sh`), so it also depends on the compose files and that script. Playwright + Chromium run in an **ephemeral venv outside `uv.lock`**, at their **own pinned `playwright`/`chromium` versions in `ci.yml`** (not Dependabot-managed — bump together on a browser refresh). Editing an Alpine component or the compose stack means this job is what proves it still works in a browser.
