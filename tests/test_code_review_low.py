...
from app.auth import BCRYPT_ROUNDS, _DUMMY_HASH, hash_password
...
def _ext(**kw):
    ...
    defaults = dict(
        id=1, store="chrome", extension_id="a" * 32, name="E", publisher="P",
        version="1", store_url="https://x", permissions="[]", risk_score=10,
    )
    defaults.update(kw)
