...
### CI gates
`.github/workflows/ci.yml` runs four **blocking** jobs on every PR + push to main: **test** (pytest, with a `postgres:16-alpine` service container + `ICEBERG_EBS_TEST_DATABASE_URL`), **lint** (`uv lock --check` + `ruff check` + `ruff format --check`), **types** (`mypy app`), **security** (`bandit -c pyproject.toml -r app` + `pip-audit`). Every job installs with `astral-sh/setup-uv` + `uv sync --locked` and runs the tool via `uv run`. Tool config and dependencies both live in `pyproject.toml` (see Dependency management above). Notes:
